...
export function registration(_options: any): Rule {
...
export function installMaterial(): Rule { 
  return (tree: Tree, _context: SchematicContext): Tree => {
    ...
    const materialDepName = '@angular/flex-layout';
            ...
            if (property.getFullText().includes('dependencies')) {
              property.forEachChild(dependency => {
                if (dependency.getFullText().includes(materialDepName)) {
                  _context.logger.info('Angular Flex-Layout already installed');
    ...
    if (!materialInstalled) {
      ...
      _context.logger.info('Installing Angular Flex-Layout');
    }
...
function addMaterial(): Rule {
  return (tree: Tree, _context: SchematicContext): Tree => {
    if(materialInstallTaskId != undefined){
      const options = {
        theme: 'indigo-pink',
        gestures: true,
        animations: true
      };
      _context.addTask(new RunSchematicTask('@angular/flex-layout', 'ng-add', options), [materialInstallTaskId]);
      _context.logger.info('Configuring Angular Flex-Layout');
    }
    return tree;
  }
}
